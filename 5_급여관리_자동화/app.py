...
if menu == "🏠 대시보드":
    ...
    if unset_employees:
        ...
        st.info("💡 '⚙️ 급여 설정' 메뉴에서 급여 정보를 입력하세요.")
    else:
...
# ============================================================
    ...
    if selected:
        ...
        # 포괄임금제 설정은 Form 밖에서 처리 (즉시 반응)
        st.markdown("#### 🏢 근무형태 및 포괄임금제 설정")
        st.caption("⚠️ 이 섹션은 즉시 반영됩니다. 설정 후 아래 Form을 작성하세요.")
        ...
        with col_pre1:
            ...
            if existing_setting:
                saved_work_type = existing_setting.get('work_type', '사무실 출퇴근')
                if saved_work_type == '재택근무 (간주근로시간제)':
                    default_work_type_index = 1
            
            work_type = st.selectbox(
                "🏢 근무형태",
                ["사무실 출퇴근", "재택근무 (간주근로시간제)"],
                index=default_work_type_index,
                help="재택근무자는 간주근로시간제 적용으로 연장근로수당 없음",
                key=f"work_type_{emp_id}"
            )
        
        with col_pre2:
            # 포괄임금제 체크박스 (사무실 출퇴근자만)
            if work_type == "사무실 출퇴근":
                is_inclusive_wage = st.checkbox(
                    ...
                    key=f"is_inclusive_{emp_id}"
                )
            else:
                is_inclusive_wage = False
                st.info("💡 간주근로시간제 적용")
        
        # 포괄임금제 적용 시 OT 입력 (Form 밖에서 즉시 반응)
        if is_inclusive_wage:
            st.markdown("**💼 고정 OT 설정**")
            st.caption("시간 또는 금액 중 하나만 입력하세요 (금액 우선 적용)")
            
            col_ot1, col_ot2 = st.columns(2)
            
            # 기본급과 시간급 계산 (Form 밖에서 미리 계산)
            temp_base_salary = existing_setting['base_salary'] if existing_setting else 2500000
            temp_work_hours = existing_setting['work_hours'] if existing_setting else 209
            
            # 정기적 고정 수당 합계 (기존 설정에서 가져오기)
            temp_regular_allowances = 0
            if existing_setting and 'allowances' in existing_setting:
                existing_allowances = existing_setting['allowances']
                temp_regular_allowances = sum([
                    v for k, v in existing_allowances.items()
                    if k in ['식대', '교통비', '직급수당', '직책수당']
                ])
            
            temp_hourly_wage = calculate_hourly_wage(temp_base_salary, temp_work_hours, temp_regular_allowances)
            ...
            with col_ot1:
                fixed_ot_hours = st.number_input(
                    ...
                    step=1.0,
                    help="기본급에 포함된 월 시간외 근무 시간",
                    key=f"fixed_ot_hours_{emp_id}"
                )
            ...
            with col_ot2:
                fixed_ot_amount = st.number_input(
                    ...
                    step=10000,
                    help="실제 지급되는 고정 OT 금액 (우선 적용)",
                    key=f"fixed_ot_amount_{emp_id}"
                )
            
            # 자동 계산 표시
            if fixed_ot_amount > 0 and temp_hourly_wage > 0:
                calc_hours = fixed_ot_amount / (temp_hourly_wage * 1.5)
                st.success(f"⏰ **자동 계산**: {C.format_currency(fixed_ot_amount)} ÷ ({C.format_currency(temp_hourly_wage)} × 1.5) = 약 {calc_hours:.1f}시간")
            elif fixed_ot_hours > 0 and temp_hourly_wage > 0:
                calc_amount = int(temp_hourly_wage * fixed_ot_hours * 1.5)
                st.success(f"💰 **자동 계산**: {C.format_currency(temp_hourly_wage)} × {fixed_ot_hours}시간 × 1.5 = {C.format_currency(calc_amount)}")
        else:
            fixed_ot_hours = 0.0
            fixed_ot_amount = 0
        
        st.divider()
        st.markdown("### 📝 급여 상세 설정")
        
        with st.form(f"payroll_setting_{emp_id}"):
            ...
            with col1:
                ...
                base_salary = st.number_input(
                    "기본급 (원)",
                    min_value=0,
                    value=existing_setting['base_salary'] if existing_setting else 2500000,
                    step=100000,
                    help="월 기본급"
                )
                ...
                # 시간급 계산 (기존 수당 반영)
                form_regular_allowances = 0
                if existing_setting and 'allowances' in existing_setting:
                    existing_allowances = existing_setting['allowances']
                    form_regular_allowances = sum([
                        v for k, v in existing_allowances.items()
                        if k in ['식대', '교통비', '직급수당', '직책수당']
                    ])
                
                hourly_wage = calculate_hourly_wage(base_salary, work_hours, form_regular_allowances)
                st.info(f"💡 **시간급 (통상임금 기준)**: {C.format_currency(hourly_wage)}")
                ...
                st.info(f"💼 **월 DC 퇴직연금**: {C.format_currency(dc_pension_amount)}")
                ...
                with col_ins1:
                    ...
                    apply_health = st.checkbox(
                        ...
                        help="직계가족은 피부양자 등록 가능, 장기요양보험 자동 포함"
                    )
                with col_ins2:
                    apply_employment = st.checkbox(
                        "고용보험",
                        value=existing_setting.get('apply_employment', True) if existing_setting else True,
                        help="만 65세 이상, 직계가족 등은 제외 가능"
                    )
                
                # 장기요양은 건강보험과 동일하게 적용
                apply_longterm = apply_health
                
                st.info("💼 **산재보험**: 전액 사업주 부담 (근로자 부담 없음)")
                
                if not all([apply_pension, apply_health, apply_employment]):
                    st.info("💡 5인 미만 사업장 직계가족 등 특례 적용")
            
            with col2:
                st.markdown("#### 🎁 수당 설정")
                
                # 기존 수당 불러오기
                existing_allowances = existing_setting['allowances'] if existing_setting else {}
                existing_tax_free = existing_setting['tax_free_items'] if existing_setting else {}
                ...
                with col_b:
                    meal_tax_free = st.checkbox(
                        "비과세",
                        value=existing_tax_free.get('식대', True),
                        key="meal_tax_free"
                    )
                if meal_allowance > 0:
                    allowances['식대'] = meal_allowance
                    tax_free_items['식대'] = meal_tax_free
                ...
                with col_a:
                    transport_allowance = st.number_input(
                        "교통비",
                        min_value=0,
                        value=existing_allowances.get('교통비', 0),
                        step=10000
                    )
                with col_b:
                    transport_tax_free = st.checkbox(
                        "비과세",
                        value=existing_tax_free.get('교통비', True),
                        key="transport_tax_free"
                    )
                if transport_allowance > 0:
                    allowances['교통비'] = transport_allowance
                    tax_free_items['교통비'] = transport_tax_free
                
                # 기타 수당
                other_allowance = st.number_input(
                    "기타 수당",
                    min_value=0,
                    value=existing_allowances.get('기타수당', 0),
                    step=10000
                )
                if other_allowance > 0:
                    allowances['기타수당'] = other_allowance
            
            submitted = st.form_submit_button("💾 저장", use_container_width=True, type="primary")
            
            if submitted:
                # 최저임금 검증
                total_monthly_pay = base_salary + sum(allowances.values())
                minimum_wage_check = validate_minimum_wage(total_monthly_pay, work_hours)
                
                if not minimum_wage_check['적법여부']:
                    ...
                    - 현재 시급: {C.format_currency(minimum_wage_check['시간급'])}
                    - 최저시급: {C.format_currency(minimum_wage_check['최저시급'])} (2026년)
                    - 최저월급: {C.format_currency(minimum_wage_check['최저월급'])}
                    
                    💡 **해결 방법**: 기본급 또는 수당을 인상하거나 근로시간을 조정하세요.
                    """)
                    st.stop()
                else:
                    st.success(f"✅ 최저임금 준수 (시급: {C.format_currency(minimum_wage_check['시간급'])})")
                ...
                settings = {
                    ...
                    'apply_longterm': apply_longterm,
                    'apply_employment': apply_employment,
                    'dc_pension_rate': dc_pension_rate,
                    'dc_pension_amount': dc_pension_amount
                }
                
                # 급여 설정 저장
                setting_saved = False
                if existing_setting:
                    setting_saved = update_payroll_setting(emp_id, settings)
                    if setting_saved:
                        st.success("✅ 급여 설정이 수정되었습니다!")
                    else:
                        st.error("❌ 급여 설정 수정 실패")
                else:
                    setting_saved = add_payroll_setting(emp_id, settings)
                    if setting_saved:
                        st.success("✅ 급여 설정이 추가되었습니다!")
                    else:
                        st.error("❌ 급여 설정 추가 실패")
                
                # 급여 설정 저장 성공 시, 자동으로 급여 이력도 생성
                if setting_saved:
                    # 현재 년월의 급여 자동 계산
                    ...
                    if add_payroll_history(emp_id, payroll_history_data, year_month):
                        st.success(f"✅ {year_month} 급여 이력이 자동 생성되었습니다!")
                    
                    st.session_state['show_payroll_summary'] = True
                    st.session_state['summary_emp_id'] = emp_id
                    st.rerun()
        
        # 저장 후 급여 설정 요약 표시
        if st.session_state.get('show_payroll_summary') and st.session_state.get('summary_emp_id') == emp_id:
            st.divider()
            st.markdown("### 📊 급여 설정 요약")
            
            saved_setting = get_payroll_setting(emp_id)
            ...
            if saved_setting:
                ...
                with col1:
                    st.markdown("#### 💵 기본 정보")
                    st.metric("기본급", C.format_currency(saved_setting['base_salary']))
                    st.metric("시간급", C.format_currency(saved_setting['hourly_wage']))
                    st.metric("월 근로시간", f"{saved_setting['work_hours']}시간")
                    st.caption(f"🏢 {saved_setting.get('work_type', '사무실 출퇴근')}")
                
                with col2:
                    st.markdown("#### 🎁 수당")
                    allowance_detail = saved_setting.get('allowances', {})
                    tax_free_detail = saved_setting.get('tax_free_items', {})
                    
                    # 고정 OT를 포함한 총 수당 계산
                    base_allowance = sum(allowance_detail.values())
                    fixed_ot = saved_setting.get('fixed_ot_amount', 0)
                    total_allowance_with_ot = base_allowance + fixed_ot
                    
                    st.metric("총 수당", C.format_currency(total_allowance_with_ot))
                    
                    # 일반 수당 표시
                    for name, amount in allowance_detail.items():
                        tax_status = "비과세" if tax_free_detail.get(name, False) else "과세"
                        st.text(f"{name}: {C.format_currency(amount)} ({tax_status})")
                    
                    # 고정 OT 표시 (수당 목록에 포함)
                    if fixed_ot > 0:
                        st.text(f"연장근로수당: {C.format_currency(fixed_ot)} (과세)")
                        st.caption(f"⏰ 고정 OT {saved_setting.get('fixed_ot_hours', 0):.1f}시간")
                
                with col3:
                    st.markdown("#### 🏥 사회보험 적용")
                    insurance_list = []
                    if saved_setting.get('apply_pension', True):
                        insurance_list.append("✅ 국민연금")
                    else:
                        insurance_list.append("❌ 국민연금")
                    
                    if saved_setting.get('apply_health', True):
                        insurance_list.append("✅ 건강보험 (장기요양 포함)")
                    else:
                        insurance_list.append("❌ 건강보험")
                    
                    if saved_setting.get('apply_employment', True):
                        insurance_list.append("✅ 고용보험")
                    else:
                        insurance_list.append("❌ 고용보험")
                    ...
                    st.divider()
                    dc_pension_amount = saved_setting.get('dc_pension_amount', 0)
                    if dc_pension_amount == 0 and saved_setting.get('base_salary', 0) > 0:
                        dc_pension_amount = saved_setting['base_salary'] * (saved_setting.get('dc_pension_rate', 8.33) / 100)
                    st.metric("DC 퇴직연금", C.format_currency(dc_pension_amount))
                    st.caption(f"{saved_setting.get('dc_pension_rate', 8.33):.2f}%")
                
                # 급여 계산 미리보기
                st.divider()
                st.markdown("### 💰 급여 계산 미리보기")
                
                # 계산
                calc_result = st.session_state.payroll_calculator.calculate_all(
                    base_salary=saved_setting['base_salary'],
                    allowances=saved_setting['allowances'],
                    tax_free_items=saved_setting['tax_free_items'],
                    apply_pension=saved_setting.get('apply_pension', True),
                    apply_health=saved_setting.get('apply_health', True),
                    apply_longterm=saved_setting.get('apply_longterm', True),
                    apply_employment=saved_setting.get('apply_employment', True),
                    fixed_ot_amount=saved_setting.get('fixed_ot_amount', 0)
                )
                ...
                with col3:
                    st.metric("실수령액", C.format_currency(calc_result['실수령액']), delta=None)
                
                # 닫기 버튼
                if st.button("✖️ 요약 닫기"):
                    st.session_state['show_payroll_summary'] = False
                    st.rerun()
...
# ============================================================
    ...
    if selected:
        ...
        if not setting:
            st.warning("⚠️ 급여 설정이 없습니다. '⚙️ 급여 설정' 메뉴에서 먼저 설정하세요.")
        else:
            # 일할계산 옵션
            st.markdown("#### 📅 일할계산")
...
# ============================================================
    ...
    if selected:
        ...
        if not setting:
            st.warning("⚠️ 급여 설정이 없습니다. 시간급을 계산할 수 없습니다.")
        else:
            ...
            if is_inclusive_wage and fixed_ot_hours > 0:
                st.info(f"💡 **{employee['name']}님의 시간급**: {C.format_currency(hourly_wage)} | 🔵 **포괄임금제 적용** (고정 OT: {fixed_ot_hours}시간)")
                st.caption("※ 고정 OT를 초과한 시간외 근무만 추가 수당으로 지급됩니다.")
            else:
                st.info(f"💡 **{employee['name']}님의 시간급**: {C.format_currency(hourly_wage)}")
            ...
            with st.form("overtime_form"):
                ...
                with col3:
                    hours = st.number_input("실제 근무 시간", min_value=0.0, max_value=24.0, value=2.0, step=0.5,
                ...
                if is_inclusive_wage and fixed_ot_hours > 0:
                    ...
                    if hours <= remaining_fixed_ot:
                        ...
                        - 이번 달 누적 시간외 근무: {total_overtime_this_month}시간
                        - 고정 OT: {fixed_ot_hours}시간
                        - 남은 고정 OT: {remaining_fixed_ot}시간
                        - 이번 등록: {hours}시간 (고정 OT 범위 내)
                        - **추가 수당**: 없음 (이미 급여에 포함됨)
                        """)
                    else:
                        # 고정 OT 초과
                        ...
                        - 남은 고정 OT: {remaining_fixed_ot}시간
                        - 이번 등록: {hours}시간
                        - **추가 수당 대상**: {billable_hours}시간
                        """)
                else:
                    billable_hours = hours
                
                # 수당 미리보기
                overtime_pay = calculate_overtime_pay(hourly_wage, billable_hours, overtime_type)
                
                st.markdown(f"""
                ### 💰 예상 시간외 수당
                
                - 기본 시간급: {C.format_currency(hourly_wage)}
                - 근무 시간: {hours}시간
                - 가산율: {C.WORK_TIME.get(f"{overtime_type}근로", {}).get("가산율", 1.5)} 배
    ...
    if selected:
        ...
        if not payroll:
            ...
            payslip_html += f"""
                    ...
                    <table class="calc-section">
                        ...
                        <tr>
                            ...
                            <td class="amount">{setting.get('hourly_wage', 0):,.0f}</td>
                            <td colspan="4"></td>
                        </tr>
                    </table>
            ...
            if setting and setting.get('is_inclusive_wage') and setting.get('fixed_ot_hours', 0) > 0:
                fixed_ot_hours = setting.get('fixed_ot_hours', 0)
                hourly_wage = setting.get('hourly_wage', 0)
                fixed_ot_amount = setting.get('fixed_ot_amount', 0)
                payslip_html += f"""
                        <tr>
                            <td class="header-cell">연장근로수당</td>
                            <td>{hourly_wage:,.0f}원 (통상시급) × {fixed_ot_hours:.1f}시간 × 1.5배 = {fixed_ot_amount:,}원</td>
                        </tr>"""
